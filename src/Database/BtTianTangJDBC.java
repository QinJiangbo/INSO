/**  
 * @description JDBC for www.bttiantang.com
...
public class BtTianTangJDBC implements DBOperator {
	
	IDbHelper db = new IDbHelperImpl();
	
	/**
	 * insert data of BtTianTang.com to the database
	 */
	@SuppressWarnings("rawtypes")
	public void insertDB() {
		String path = Config.BtTianTangExtractionCache;
		...
		if(dir.exists()) {
			...
			for(int i = 0; i < fileList.length; i++) {
				...
				if(!BtTianTangURLExist(data.get("URL").toString())) {
					String sql = "insert into bttiantang(Title, Path, URL, Content, PublishTime, UpdateTime) values(?,?,?,?,?,?)";
			...
			copyFiles(Config.BtTianTangExtractionCache, Config.BtTianTangExtraction);
			FileIO.deleteFiles(Config.BtTianTangExtractionCache);
		}
	}
	...
	public Map[] selectURL() {
		String sql = "select URL from bttiantang";
		Map[] rows = this.db.runSelect(sql);
		return rows;
	}
	
	/**
	 * delete the records stored in the BtTianTang table
	 */
	public void deleteBtTianTangRecord() {
		String sql = "delete from btbianbang";
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public boolean BtTianTangURLExist(String URL) {
		//long start = System.currentTimeMillis();
		String sql = "select count(*) n from bttiantang where URL = ?";
		Object[] params = { URL };
