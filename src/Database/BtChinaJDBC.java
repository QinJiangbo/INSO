/**  
 * @description JDBC for henbt.com
...
public class BtChinaJDBC implements DBOperator {
	...
	public void insertDB() {
		String path = Config.BtChinaExtractionCache;
		...
		if(dir.exists()) {
			...
			for(int i = 0; i < fileList.length; i++) {
				...
				if(!BtChinaURLExist(data.get("URL").toString())) {
					String sql = "insert into btchina(Title, Path, URL, Content, PublishTime, UpdateTime) values(?,?,?,?,?,?)";
			...
			copyFiles(Config.BtChinaExtractionCache, Config.BtChinaExtraction);
			FileIO.deleteFiles(Config.BtChinaExtractionCache);
		}
	}
	...
	public Map[] selectURL() {
		String sql = "select URL from btchina";
		Map[] rows = this.db.runSelect(sql);
		return rows;
	}
	
	/**
	 * delete the records stored in the BtChina table
	 */
	public void deleteBtChinaRecord() {
		String sql = "delete from btchina";
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public boolean BtChinaURLExist(String URL) {
		//long start = System.currentTimeMillis();
		String sql = "select count(*) n from btchina where URL = ?";
		Object[] params = { URL };
