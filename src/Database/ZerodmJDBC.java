/**  
 * @description JDBC for dmxz.89dm.com
...
public class ZerodmJDBC implements DBOperator {
	
	IDbHelper db = new IDbHelperImpl();
	
	/**
	 * insert data of dmxz.89dm.com to the database
	 */
	@SuppressWarnings("rawtypes")
	public void insertDB() {
		String path = Config.ZerodmExtractionCache;
		...
		if(dir.exists()) {
			...
			for(int i = 0; i < fileList.length; i++) {
				...
				if(!ZerodmURLExist(data.get("URL").toString())) {
					String sql = "insert into zerodm(Title, Path, URL, Content, PublishTime, UpdateTime) values(?,?,?,?,?,?)";
			...
			copyFiles(Config.ZerodmExtractionCache, Config.ZerodmExtraction);
			FileIO.deleteFiles(Config.ZerodmExtractionCache);
		}
	}
	...
	public Map[] selectURL() {
		String sql = "select URL from zerodm";
		Map[] rows = this.db.runSelect(sql);
		return rows;
	}
	
	/**
	 * delete the records stored in the Zerodm table
	 */
	public void deleteZerodmRecord() {
		String sql = "delete from zerodm";
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public boolean ZerodmURLExist(String URL) {
		//long start = System.currentTimeMillis();
		String sql = "select count(*) n from zerodm where URL = ?";
		Object[] params = { URL };
