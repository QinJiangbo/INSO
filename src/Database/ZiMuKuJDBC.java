/**  
 * @description JDBC for www.zimuku.net
...
public class ZiMuKuJDBC implements DBOperator {
	
	IDbHelper db = new IDbHelperImpl();
	
	/**
	 * insert data of www.zimuku.net to the database
	 */
	@SuppressWarnings("rawtypes")
	public void insertDB() {
		String path = Config.ZiMuKuExtractionCache;
		...
		if(dir.exists()) {
			...
			for(int i = 0; i < fileList.length; i++) {
				...
				if(!ZiMuKuURLExist(data.get("URL").toString())) {
					String sql = "insert into zimuku(Title, Path, URL, Content, PublishTime, UpdateTime) values(?,?,?,?,?,?)";
			...
			copyFiles(Config.ZiMuKuExtractionCache, Config.ZiMuKuExtraction);
			FileIO.deleteFiles(Config.ZiMuKuExtractionCache);
		}
	}
	...
	public Map[] selectURL() {
		String sql = "select URL from zimuku";
		Map[] rows = this.db.runSelect(sql);
		return rows;
	}
	
	/**
	 * delete the records stored in the ZiMuKu table
	 */
	public void deleteZiMuKuRecord() {
		String sql = "delete from zimuku";
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public boolean ZiMuKuURLExist(String URL) {
		//long start = System.currentTimeMillis();
		String sql = "select count(*) n from zimuku where URL = ?";
		Object[] params = { URL };
