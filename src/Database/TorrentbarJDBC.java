/**  
 * @description JDBC for www.torrentbar.com
...
public class TorrentbarJDBC implements DBOperator {
	
	IDbHelper db = new IDbHelperImpl();
	
	/**
	 * insert data of www.torrentbar.com to the database
	 */
	@SuppressWarnings("rawtypes")
	public void insertDB() {
		String path = Config.TorrentbarExtractionCache;
		...
		if(dir.exists()) {
			...
			for(int i = 0; i < fileList.length; i++) {
				...
				if(!TorrentbarURLExist(data.get("URL").toString())) {
					String sql = "insert into torrentbar(Title, Path, URL, Content, PublishTime, UpdateTime) values(?,?,?,?,?,?)";
			...
			copyFiles(Config.TorrentbarExtractionCache, Config.TorrentbarExtraction);
			FileIO.deleteFiles(Config.TorrentbarExtractionCache);
		}
	}
	...
	public Map[] selectURL() {
		String sql = "select URL from torrentbar";
		Map[] rows = this.db.runSelect(sql);
		return rows;
	}
	
	/**
	 * delete the records stored in the Torrentbar table
	 */
	public void deleteTorrentbarRecord() {
		String sql = "delete from torrentbar";
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public boolean TorrentbarURLExist(String URL) {
		//long start = System.currentTimeMillis();
		String sql = "select count(*) n from torrentbar where URL = ?";
		Object[] params = { URL };
