...
public class YifyJDBC implements DBOperator {
	...
	public void insertDB() {
		String path = Config.YifyExtractionCache;
		...
		if(dir.exists()) {
			...
			for(int i = 0; i < fileList.length; i++) {
				...
				if(!YifyURLExist(data.get("URL").toString())) {
					String sql = "insert into yify(Title, Path, URL, Content, PublishTime, UpdateTime) values(?,?,?,?,?,?)";
			...
			copyFiles(Config.YifyExtractionCache, Config.YifyExtraction);
			FileIO.deleteFiles(Config.YifyExtractionCache);
		}
	}
	...
	public Map[] selectURL() {
		String sql = "select URL from yify";
		Map[] rows = this.db.runSelect(sql);
		return rows;
	}
	
	/**
	 * delete the records stored in the Yify table
	 */
	public void deleteYifyRecord() {
		String sql = "delete from yify";
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public boolean YifyURLExist(String URL) {
		//long start = System.currentTimeMillis();
		String sql = "select count(*) n from yify where URL = ?";
		Object[] params = { URL };
