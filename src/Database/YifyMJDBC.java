...
public class YifyMJDBC implements DBOperator {
	...
	public void insertDB() {
		String path = Config.YifyMExtractionCache;
		...
		if(dir.exists()) {
			...
			for(int i = 0; i < fileList.length; i++) {
				...
				if(!YifyMURLExist(data.get("URL").toString())) {
					String sql = "insert into yifym(Title, Path, URL, Content, PublishTime, UpdateTime) values(?,?,?,?,?,?)";
			...
			copyFiles(Config.YifyMExtractionCache, Config.YifyMExtraction);
			FileIO.deleteFiles(Config.YifyMExtractionCache);
		}
	}
	...
	public Map[] selectURL() {
		String sql = "select URL from yifym";
		Map[] rows = this.db.runSelect(sql);
		return rows;
	}
	
	/**
	 * delete the records stored in the YifyM table
	 */
	public void deleteYifyMRecord() {
		String sql = "delete from yifym";
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public boolean YifyMURLExist(String URL) {
		//long start = System.currentTimeMillis();
		String sql = "select count(*) n from yifym where URL = ?";
		Object[] params = { URL };
