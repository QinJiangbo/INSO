/**  
 * @description JDBC for dm1080p.com
...
public class DM1080pJDBC implements DBOperator {
	
	IDbHelper db = new IDbHelperImpl();
	
	/**
	 * insert data of dm1080p.com to the database
	 */
	@SuppressWarnings("rawtypes")
	public void insertDB() {
		String path = Config.DM1080pExtractionCache;
		...
		if(dir.exists()) {
			...
			for(int i = 0; i < fileList.length; i++) {
				...
				if(!DM1080pURLExist(data.get("URL").toString())) {
					String sql = "insert into dm1080p(Title, Path, URL, Content, PublishTime, UpdateTime) values(?,?,?,?,?,?)";
			...
			copyFiles(Config.DM1080pExtractionCache, Config.DM1080pExtraction);
			FileIO.deleteFiles(Config.DM1080pExtractionCache);
		}
	}
	...
	public Map[] selectURL() {
		String sql = "select URL from dm1080p";
		Map[] rows = this.db.runSelect(sql);
		return rows;
	}
	
	/**
	 * delete the records stored in the DM1080p table
	 */
	public void deleteDM1080pRecord() {
		String sql = "delete from dm1080p";
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public boolean DM1080pURLExist(String URL) {
		//long start = System.currentTimeMillis();
		String sql = "select count(*) n from dm1080p where URL = ?";
		Object[] params = { URL };
