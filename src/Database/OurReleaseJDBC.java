/**  
 * @description JDBC for www.ourrelease.org
 * @author QinJiangbo 2012302580314
 * @version 1.0
 * @date Apr 25, 2015 8:39:11 PM
 */
...
public class OurReleaseJDBC implements DBOperator {
	
	IDbHelper db = new IDbHelperImpl();
	
	/**
	 * insert data of OurRelease.org to the database
	 */
	@SuppressWarnings("rawtypes")
	public void insertDB() {
		String path = Config.OurReleaseExtractionCache;
		...
		if(dir.exists()) {
			...
			for(int i = 0; i < fileList.length; i++) {
				...
				if(!OurReleaseURLExist(data.get("URL").toString())) {
					String sql = "insert into ourrelease(Title, Path, URL, Content, PublishTime, UpdateTime) values(?,?,?,?,?,?)";
			...
			copyFiles(Config.OurReleaseExtractionCache, Config.OurReleaseExtraction);
			FileIO.deleteFiles(Config.OurReleaseExtractionCache);
		}
	}
	...
	public Map[] selectURL() {
		String sql = "select URL from ourrelease";
		Map[] rows = this.db.runSelect(sql);
		return rows;
	}
	
	/**
	 * delete the records stored in the OurRelease table
	 */
	public void deleteOurReleaseRecord() {
		String sql = "delete from ourrelease";
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public boolean OurReleaseURLExist(String URL) {
		//long start = System.currentTimeMillis();
		String sql = "select count(*) n from ourrelease where URL = ?";
		Object[] params = { URL };
