/**  
 * @description Extract files from www.yify-torrent.org
...
public class YifyExtractor extends FilmExtractor {
	 ...
	 */
	public int getSize(){
		File htmlDir = new File(Config.YifyDownloadCache);
		if(!htmlDir.exists()) {
			htmlDir.mkdir();
		}
	...
	public void extract() {
		...
		File YifyExtraction = new File(Config.YifyExtractionCache);
		if(!YifyExtraction.exists()) {
			YifyExtraction.mkdir();
		}
		
		for(int i = 0; i < fileList.length; i++) {
			String fileName = fileList[i].getName();
			if(!fileExist(Config.YifyDownloadCache + fileList[i].getName())) {
				System.out.println(Config.YifyDownloadCache + fileList[i].getName());
				fileName = fileName.substring(0, fileName.length()-5);
				String originalUrl = (Config.YifyURL + fileList[i].getName()).replaceAll("_", "/");
				String filmPath = Config.YifyDownload + fileList[i].getName();
				if(this.extractContent(Config.YifyDownloadCache + fileList[i].getName()) != "") {
					String result = originalUrl + "\n" + filmPath + "\n" + this.extractContent(Config.YifyDownloadCache + fileList[i].getName());
					FileIO.writeFile(Config.YifyExtractionCache + fileName + ".txt", result);
				}
				else {
					FileIO.deleteFile(Config.YifyDownloadCache + fileList[i].getName());
				}
			}else {
				System.out.println(Config.YifyExtractionCache + fileName.substring(0, fileName.length()-5) + ".txt already exist!");
			}
			count = count + 1;
		}
		
		FileIO.copyFiles(Config.YifyDownloadCache, Config.YifyDownload);
		FileIO.deleteFiles(Config.YifyDownloadCache);
	}
	...
	public boolean fileExist(String path) {
		String sql = "select count(*) n from yify where Path = ?";
		Object[] params = {path};
